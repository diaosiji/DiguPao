...
- (NSString *)created_at {
    ...
    if ([createdDate isThisYear]) {// 今年
        if ([createdDate isThisYesterday]) {// 昨天
            formatter.dateFormat = @"收藏于昨天 HH:mm";
            return [formatter stringFromDate:createdDate];
        } else if ([createdDate isThisToday]) {//今天 比较复杂
            if (components.hour >= 1) {// 大于1小时前发的
                return [NSString stringWithFormat:@"收藏于%ld小时前", (long)components.hour];
            } else if (components.minute >= 1){
                return [NSString stringWithFormat:@"收藏于%ld分钟前", (long)components.minute];
            } else {// 一分钟内
                return @"刚刚收藏";
            }
        } else {// 其他
            formatter.dateFormat = @"收藏于MM-dd HH:mm";
            return [formatter stringFromDate:createdDate];
            
        }
    } else {// 非今年
        formatter.dateFormat = @"收藏于yyyy-MM-dd HH:mm";
        return [formatter stringFromDate:createdDate];
    }
